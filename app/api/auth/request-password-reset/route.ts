...
export async function POST(request: Request) {
  ...
  const payload = (await request.json()) as RequestPasswordResetPayload;
  ...
  if (generateLinkResult.error) {
    console.warn("Password reset generateLink failed.", generateLinkResult.error.message);
    return NextResponse.json({ ok: true });
  }
  ...
  try {
    ...
    if (error instanceof InviteMailerConfigError) {
      console.warn("Password reset email skipped due to missing mailer config.");
    } else {
