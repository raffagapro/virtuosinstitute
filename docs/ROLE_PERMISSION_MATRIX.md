...
| View parent sensitive fields (`CURP`, `RFC`, profession, invoice flag) | `manage` | `manage` | `view` | `view` | `none` | `limited` | `manage` | `none` |
...
Notes:
- `clerk` may need limited visibility for operational verification, but not unrestricted editing.
- If preferred, clerk access to `CURP`/`RFC` can be reduced further during implementation.
...
- Teacher visibility is for teaching context; teacher edit rights stay narrow.
...
These items are narrowed enough to implement later, but still worth confirming before RLS and UI work begins:

- Whether `clerk` should see parent `CURP`/`RFC`, or only document verification status.
- Whether `teacher` should see full allergy/medical detail or a reduced safety summary.
- Whether `direction` and `coordination` have equal approval power, or one is primary.
- Whether `superadmin` should act in school workflows directly or stay mostly platform-only.